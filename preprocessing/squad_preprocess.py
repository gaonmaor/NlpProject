...
def tokenize(sequence):
    ...
    #m=list(map(lambda x:x.encode('utf8'), tokens))
    return tokens
...
def save_files(prefix, tier, indices,context,question,text,span):
    with open(os.path.join(prefix, tier + '.context'), 'w') as context_file,  \
     open(os.path.join(prefix, tier + '.question'), 'w') as question_file,\
        open(os.path.join(prefix, tier + '.answer'), 'w') as text_file, \
        open(os.path.join(prefix, tier + '.span'), 'w') as span_file:
            for i in indices:    
                context_file.write(context[i-1])
                question_file.write(question[i-1])
                text_file.write(text[i-1])
                span_file.write(span[i-1])
...
def split_tier(prefix, train_percentage = 0.9, shuffle=False):
    ...
    context_f=open(os.path.join(prefix, 'train.context'),'r')
    context=context_f.readlines()
    question_f=open(os.path.join(prefix, 'train.question'),'r')
    question=question_f.readlines()
    text_f=open(os.path.join(prefix, 'train.answer'),'r')
    text= text_f.readlines()
    span_f=open(os.path.join(prefix, 'train.span'),'r')
    span= span_f.readlines()
            
    print(len(span))
    save_files(prefix, 'val', indices_dev,context,question,text,span)
    indices_train = list(range(num_lines))[:int(num_lines * train_percentage)]
    if shuffle:
        np.random.shuffle(indices_train)
    print(len(span))
    save_files(prefix, 'train', indices_train,context,question,text,span)
...
if __name__ == '__main__':
    ...
    #train_dataset = maybe_download(squad_base_url, train_filename, prefix, bytes(30288272))
    ...
    #dev_dataset = maybe_download(squad_base_url, dev_filename, prefix, bytes(4854279))
